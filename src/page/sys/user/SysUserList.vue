...
import SysUserEdit from './SysUserEdit'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: parameter => {
        return service.sysUserService.pageQuery(parameter).then(res => {
          return res
        }
        )
      }
  ...
  methods: {
    ...
    delAll () {
      this.$service.sysUserService.batchDelIds(this.$refs.table.rowSelection.selectedRowKeys.toString()).then(res => {
